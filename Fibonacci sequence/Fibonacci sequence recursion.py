# declares cache-dictionary,
# as we know the 0th and 1st Fibonacci's number
# this is also the base case of recursion
sequence = {0: 0, 1: 1}


# function first checks whether the needed number is in the dictionary,
...
# gets value from the user and converts the type from string to int
numbers = int(input("How many Fibonacci numbers you want to display? "))

# calls the function for the value provided by the user
recursion_Fibonacci(numbers)

# displays the content of the dictionary up to the value provided by the user
# when the value is 0, the entire dictionary is not displayed, but only the 0th number
for i in range(numbers + 1):
    print(sequence[i], end=" ")

# calculates the golden ratio value
# and handles an exception if the sequence is too short
try:
